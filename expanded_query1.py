...
class IdealQueryGeneration:
    ...
    def tweak_rocchio_weights(self, query: str, rocchio_vector: List[RocchioStat], alpha: float):
        ...
        tolerance_limit = 5  # Maximum allowed iterations with no improvement
        for rocchio_stat in rocchio_vector:
            term = rocchio_stat.term
            original_weight = rocchio_stat.weight

            best_weight = original_weight
            best_map = self.compute_map(query, rocchio_vector, None)  # Compute initial MAP

            # Test increasing weights
            for _ in range(tolerance_limit):
                rocchio_stat.weight *= 1.1
                current_map = self.compute_map(query, rocchio_vector, None)
                if current_map > best_map:
                    best_map = current_map
                    best_weight = rocchio_stat.weight
                else:
                    rocchio_stat.weight /= 1.1  # Revert if no improvement
                    break

            # Restore original weight
            rocchio_stat.weight = best_weight

            # Test decreasing weights
            for _ in range(tolerance_limit):
                rocchio_stat.weight *= 0.9
                current_map = self.compute_map(query, rocchio_vector, None)
                if current_map > best_map:
                    best_map = current_map
                    best_weight = rocchio_stat.weight
                else:
                    rocchio_stat.weight /= 0.9  # Revert if no improvement
                    break
    ...
    def compute_map(self, query: str, rocchio_vector: List[RocchioStat], qrel_path: str) -> float:
        ...
        expanded_query = self.expanded_query(query, rocchio_vector, alpha=1.0)
        top_docs = self.searcher.search(expanded_query, 1000)  # Retrieve top 1000 results

        retrieved_docs = [self.searcher.doc(score_doc.doc).get("docid") for score_doc in top_docs.scoreDocs]
        relevant_docs = set(self.query_rel_doc_map.get(query, []))
