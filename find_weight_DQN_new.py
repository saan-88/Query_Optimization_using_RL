...
class QueryWeightOptimizer:
    def __init__(self, stopword_path, index_dir: str, query_id=601, term_count=10,
                 num_episodes=10, max_steps=5000, step_size=0.25, buffer_size=1000, batch_size=32):
        ...
        self.q_function.fit([np.zeros(self.num_terms * 2)], [0])  # Dummy fit
    ...
    def evaluate_query(self, query):
        ...
        global search_time  # Use the global variable

        # Measure time for retrieving top 1000 documents
    ...
    def train(self):
        ...
        for episode in range(self.num_episodes):
            ...
            for step in range(self.max_steps):
                ...
                self.replay_buffer.append((current_state, action, reward, next_state))

                # **Update Q-function if enough samples in buffer**
    ...
    def update_q_function(self):
        """
        Sample a batch from replay buffer and update Q-function.
        """
        ...
        for state, action, reward, next_state in batch:
            target = reward + 0.9 * max(
                self.q_function.predict([np.concatenate([next_state, a])])[0] for a in self.get_action_space()
            )
            X_train.append(np.concatenate([state, action]))
            y_train.append(target)

        self.q_function.partial_fit(X_train, y_train)
...
optimizer.train()
print(f"Total search time: {search_time}")
